...
function buildVehicleFilterSql(string $filtroVeiculo, array &$params, string $prefix = ''): string {
    switch ($filtroVeiculo) {
        case 'moto':
            $params[] = 'MOTO';
            return " AND UPPER({$prefix}vehicle_type) = ? ";
        case 'fiorino_grupo':
            $params[] = 'FIORINO';
            $params[] = 'ZOF-FIORINO-AR';
            return " AND UPPER({$prefix}vehicle_type) IN (?, ?) ";
        case 'passeio_grupo':
            $params[] = 'PASSEIO';
            $params[] = 'ZOF-PASSEIO-AR';
            return " AND UPPER({$prefix}vehicle_type) IN (?, ?) ";
        default:
            return '';
    }
...
function buscarResumo(PDO $pdo): array {
...
function buscarRelatorioMesas(PDO $pdo): array {
    ...
    ");
    $temposRows = $stmtTempos->fetchAll(PDO::FETCH_ASSOC);
    ...
    return [
        'relatorioMesas' => $relatorioMesas,
        'totalRotasMesas' => $totalRotasMesas,
        'totalSegundosMesas' => $totalSegundosMesas,
        'tempoMedioGeral' => $tempoMedioGeral,
        'rotasPorHoraGeral' => $rotasPorHoraGeral,
    ];
}

function buscarDrivers(PDO $pdo, string $filtroStatus, string $filtroVeiculo): array {
    $sql = "SELECT * FROM drivers WHERE active = true";
    $params = [];
    ...
    $sql .= buildVehicleFilterSql($filtroVeiculo, $params);

    $sql .= " ORDER BY
        CASE
            WHEN UPPER(vehicle_type) IN ('FIORINO', 'ZOF-FIORINO-AR') THEN 1
            WHEN UPPER(vehicle_type) = 'MOTO' THEN 2
            WHEN UPPER(vehicle_type) IN ('PASSEIO', 'ZOF-PASSEIO-AR') THEN 3
            ELSE 4
        END,
        driver_name ASC";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'create_route') {
    ...
    try {
        ...
        foreach ($clustersParaSalvar as $clusterItem) {
            ...
            $stmtCluster->execute([$driverRef, $clusterCode, $packages, $ordem++]);
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'delete_route') {
    ...
    try {
        ...
        $driverDelete = $stmtBusca->fetch(PDO::FETCH_ASSOC);

        if (!$driverDelete) {
            throw new Exception('Motorista não encontrado.');
        }

        $pdo->prepare('DELETE FROM mesa_controle WHERE driver_db_id = ?')->execute([$driverDeleteId]);
        $pdo->prepare('DELETE FROM mesas_controle WHERE driver_db_id = ? OR driver_id = ?')->execute([$driverDeleteId, $driverDelete['driver_id']]);
        $pdo->prepare('DELETE FROM mesa_tempos WHERE driver_ref = ?')->execute([$driverDeleteId]);
        $pdo->prepare('DELETE FROM driver_clusters WHERE driver_ref = ?')->execute([$driverDeleteId]);
        $pdo->prepare('DELETE FROM drivers WHERE id = ?')->execute([$driverDeleteId]);
...
$filtroVeiculo = strtolower(trim($_GET['veiculo'] ?? 'todos'));
$veiculosPermitidos = ['todos', 'moto', 'fiorino_grupo', 'passeio_grupo'];
...
if (isset($_GET['ajax']) && $_GET['ajax'] === 'dashboard_data') {
    ...
    $driversAjax = buscarDrivers($pdo, $filtroStatus, $filtroVeiculo);
    $resumoAjax = buscarResumo($pdo);
    $relatorioAjax = buscarRelatorioMesas($pdo);

    $linhas = [];
    foreach ($driversAjax as $d) {
        ...
        $linhas[] = [
            ...
            'cluster_text' => (string)$d['cluster_text'],
            'packages_total' => (string)$d['packages_total'],
    ...
    echo json_encode([
        'ok' => true,
        'resumo' => array_merge($resumoAjax, [
            'totalRotasMesas' => $relatorioAjax['totalRotasMesas'],
            'tempoMedioGeral' => formatarDuracao($relatorioAjax['tempoMedioGeral']),
            'rotasPorHoraGeral' => $relatorioAjax['rotasPorHoraGeral'],
            'rotaCada' => formatarMinutosPorRota($relatorioAjax['tempoMedioGeral']),
        ]),
        'drivers' => $linhas,
...
$drivers = buscarDrivers($pdo, $filtroStatus, $filtroVeiculo);
$resumoDados = buscarResumo($pdo);
$totalGeral = $resumoDados['totalGeral'];
$totalAtivos = $resumoDados['totalAtivos'];
$totalConferindo = $resumoDados['totalConferindo'];
$totalFinalizados = $resumoDados['totalFinalizados'];

$relatorioDados = buscarRelatorioMesas($pdo);
$relatorioMesas = $relatorioDados['relatorioMesas'];
$totalRotasMesas = $relatorioDados['totalRotasMesas'];
$tempoMedioGeral = $relatorioDados['tempoMedioGeral'];
$rotasPorHoraGeral = $relatorioDados['rotasPorHoraGeral'];
...
if (isset($_GET['export']) && $_GET['export'] === 'mesas_csv') {
    ...
    fputcsv($out, [
        'Mesa', 'Qtd Rotas', 'Tempo Total', 'Tempo Medio por Rota', 'Rotas por Hora', '1 Rota a Cada', 'Rotas Feitas', 'Motoristas'
    ], ';');
...
.topo h2{margin:0;font-size:29px;font-weight:900;letter-spacing:.2px}
.topo-sub{font-size:13px;opacity:.92}
.acoes-topo{display:flex;gap:10px;flex-wrap:wrap}
.container{max-width:1500px;margin:0 auto;padding:28px}
.hero{display:grid;grid-template-columns:1.35fr .95fr;gap:18px;margin-bottom:22px}
.hero-card{border-radius:26px;padding:26px;color:#fff;position:relative;overflow:hidden;box-shadow:var(--shadow-md)}
.hero-card::after{content:"";position:absolute;inset:auto -80px -80px auto;width:220px;height:220px;background:rgba(255,255,255,.10);border-radius:50%}
.hero-main{background:linear-gradient(135deg,#ff6a3d 0%, #ee4d2d 45%, #d83b1b 100%)}
.hero-side{background:linear-gradient(135deg,#18233d 0%, #0f172a 100%)}
.hero-title{margin:0 0 8px 0;font-size:30px;font-weight:900}
.hero-text{margin:0;max-width:760px;line-height:1.55;opacity:.95}
.hero-mini-grid{display:grid;grid-template-columns:repeat(2, minmax(120px,1fr));gap:14px;margin-top:18px}
.hero-mini{background:rgba(255,255,255,.12);border:1px solid rgba(255,255,255,.14);border-radius:18px;padding:14px;backdrop-filter:blur(4px)}
.hero-mini-label{font-size:12px;opacity:.9;margin-bottom:6px}
.hero-mini-value{font-size:26px;font-weight:900}
.card{background:linear-gradient(180deg,#ffffff 0%,#fbfcfe 100%);border:1px solid var(--line);padding:22px;border-radius:var(--radius);margin-bottom:22px;box-shadow:var(--shadow-sm)}
.card:hover{box-shadow:var(--shadow-md)}
.card h3{margin:0 0 18px 0;font-size:22px;font-weight:900}
.bloco-info{display:flex;justify-content:space-between;align-items:flex-start;gap:14px;margin-bottom:16px;flex-wrap:wrap}
.bloco-info p{margin:0;color:var(--muted);font-size:14px;max-width:560px;line-height:1.45}
.btn{appearance:none;border:none;outline:none;text-decoration:none;display:inline-flex;align-items:center;justify-content:center;gap:8px;padding:12px 16px;border-radius:14px;cursor:pointer;font-size:14px;font-weight:800;color:#fff;transition:transform .15s ease, box-shadow .15s ease, opacity .15s ease;box-shadow:0 8px 18px rgba(0,0,0,.10)}
...
.btn-ok{background:linear-gradient(135deg,#16a34a 0%, #22c55e 100%)}
...
.kpi{border-radius:18px;padding:18px;border:1px solid var(--line);background:#fff;box-shadow:var(--shadow-sm)}
.kpi-label{font-size:13px;color:var(--muted);margin-bottom:8px;font-weight:800}
.kpi-value{font-size:32px;font-weight:900;color:var(--text)}
.form-area{display:grid;gap:14px}
.form-linha{display:flex;align-items:center;gap:12px;flex-wrap:wrap;padding:14px;border:1px solid var(--line);border-radius:16px;background:#fff}
.import-grid{display:grid;grid-template-columns:1fr 1fr auto;gap:14px;width:100%;align-items:end}
.input-group{display:flex;flex-direction:column;gap:8px}
.input-group label{font-size:14px;font-weight:900;color:var(--text)}
.input-group input,.input-group select,.input-group textarea{width:100%;padding:13px 14px;border:1px solid var(--line-strong);border-radius:14px;font-size:14px;color:var(--text);background:#fff;transition:border-color .15s ease, box-shadow .15s ease}
.input-group textarea{min-height:96px;resize:vertical}
.input-group input:focus,.input-group select:focus,.input-group textarea:focus{outline:none;border-color:#ffb29f;box-shadow:0 0 0 4px rgba(238,77,45,.10)}
.criacao-grid{display:grid;grid-template-columns:repeat(12, minmax(0, 1fr));gap:14px}
.span-3{grid-column:span 3}.span-4{grid-column:span 4}.span-5{grid-column:span 5}.span-6{grid-column:span 6}.span-12{grid-column:span 12}
.form-box{background:linear-gradient(180deg,#ffffff 0%,#fcfdff 100%);border:1px solid var(--line);border-radius:20px;padding:18px}
.form-box h4{margin:0 0 8px 0;font-size:18px;font-weight:900}
.form-box p{margin:0 0 14px 0;color:var(--muted);font-size:13px;line-height:1.45}
.chips-ajuda{display:flex;gap:8px;flex-wrap:wrap;margin-top:8px}
.chip-ajuda{display:inline-flex;align-items:center;padding:8px 11px;border-radius:999px;border:1px solid #e7eaf1;background:#f8fafc;color:#344054;font-size:12px;font-weight:800}
.preview-box{background:#fff7ed;border:1px solid #fed7aa;border-radius:16px;padding:12px 14px;color:#9a3412;font-size:13px;line-height:1.45}
.filtros{display:flex;gap:10px;flex-wrap:wrap;margin-bottom:18px}
.filtro-btn{text-decoration:none;padding:10px 14px;border-radius:999px;border:1px solid var(--line);background:#fff;color:var(--text);font-size:13px;font-weight:900}
.filtro-ativo{background:linear-gradient(135deg,var(--brand) 0%,var(--brand-2) 100%);color:#fff;border-color:transparent}
.table-wrap{overflow:auto;border:1px solid var(--line);border-radius:18px;background:#fff}
.table-toolbar{display:flex;justify-content:space-between;align-items:center;gap:14px;flex-wrap:wrap;margin-bottom:14px}
.table-toolbar-title{font-size:14px;color:var(--muted);font-weight:800}
table{width:100%;border-collapse:separate;border-spacing:0;min-width:1100px}
thead th{background:linear-gradient(180deg,#f9fbfd 0%,#f1f5f9 100%);color:#1f2937;font-weight:900;font-size:14px;text-align:left;padding:15px 12px;border-bottom:1px solid var(--line);white-space:nowrap}
...
.status{display:inline-flex;align-items:center;gap:8px;padding:8px 12px;border-radius:999px;font-size:12px;font-weight:900;text-transform:capitalize}
.status::before{content:"";width:8px;height:8px;border-radius:50%;display:inline-block}
.status-ativo{color:#166534;background:#ecfdf3}.status-ativo::before{background:#22c55e}
.status-conferindo{color:#92400e;background:#fff7ed}.status-conferindo::before{background:#f59e0b}
.status-finalizado{color:#475467;background:#f2f4f7}.status-finalizado::before{background:#98a2b3}
.tag{display:inline-block;padding:8px 12px;border-radius:999px;font-size:12px;font-weight:900;background:#f3f6fb;color:#344054;border:1px solid #e6ebf2}
.vazio{text-align:center;color:var(--muted);padding:24px;font-size:15px}
.acoes-tabela{display:flex;gap:8px;flex-wrap:wrap}
.btn-mini{appearance:none;border:none;cursor:pointer;text-decoration:none;display:inline-flex;align-items:center;justify-content:center;padding:10px 12px;border-radius:12px;font-size:13px;font-weight:800;color:#fff;box-shadow:0 8px 18px rgba(0,0,0,.10)}
.btn-mini.edit{background:linear-gradient(135deg,var(--blue) 0%,var(--blue-2) 100%)}
.btn-mini.delete{background:linear-gradient(135deg,var(--red) 0%,var(--red-2) 100%)}
.modal-feedback{position:fixed;inset:0;background:rgba(15,23,42,.40);display:none;align-items:center;justify-content:center;z-index:9999;padding:18px}
.modal-feedback.ativo{display:flex}
.modal-card{width:100%;max-width:480px;background:#fff;border-radius:24px;box-shadow:0 24px 60px rgba(15,23,42,.20);padding:24px;text-align:center;animation:modalIn .18s ease}
.modal-icon{width:70px;height:70px;border-radius:50%;margin:0 auto 14px auto;display:flex;align-items:center;justify-content:center;font-size:34px;font-weight:900}
.modal-icon.success{background:#ecfdf3;color:#16a34a}
.modal-icon.error{background:#fff1f2;color:#dc2626}
.modal-title{font-size:24px;font-weight:900;margin:0 0 10px 0}
.modal-text{font-size:15px;color:var(--muted);line-height:1.5;margin:0 0 18px 0;white-space:pre-line}
.modal-actions{display:flex;gap:10px;justify-content:center;flex-wrap:wrap}
@keyframes modalIn{from{opacity:0;transform:translateY(8px) scale(.98)}to{opacity:1;transform:translateY(0) scale(1)}}
@media (max-width:1100px){.hero{grid-template-columns:1fr}.kpis{grid-template-columns:repeat(2,minmax(140px,1fr))}.import-grid{grid-template-columns:1fr}.criacao-grid{grid-template-columns:1fr}.span-3,.span-4,.span-5,.span-6,.span-12{grid-column:span 1}}
@media (max-width:720px){.container{padding:16px}.topo{padding:18px 16px;flex-direction:column;align-items:flex-start}.topo h2{font-size:24px}.hero-title{font-size:24px}.kpis{grid-template-columns:1fr}.btn{width:100%}}
</style>
</head>
<body>
<div class="topo">
...
<div class="container">
    <div class="hero">
        <div class="hero-card hero-main">
            ...
            <div class="hero-mini-grid">
                <div class="hero-mini"><div class="hero-mini-label">Motoristas ativos</div><div class="hero-mini-value" id="heroTotalAtivos"><?= $totalAtivos ?></div></div>
                <div class="hero-mini"><div class="hero-mini-label">Em conferência</div><div class="hero-mini-value" id="heroTotalConferindo"><?= $totalConferindo ?></div></div>
                <div class="hero-mini"><div class="hero-mini-label">Finalizados</div><div class="hero-mini-value" id="heroTotalFinalizados"><?= $totalFinalizados ?></div></div>
                <div class="hero-mini"><div class="hero-mini-label">Rotas registradas</div><div class="hero-mini-value" id="heroTotalRotasMesas"><?= $totalRotasMesas ?></div></div>
            </div>
        </div>
        ...
        <div class="hero-card hero-side">
            ...
            <div class="hero-mini-grid">
                <div class="hero-mini"><div class="hero-mini-label">Tempo médio geral</div><div class="hero-mini-value" style="font-size:22px;" id="heroTempoMedioGeral"><?= formatarDuracao($tempoMedioGeral) ?></div></div>
                <div class="hero-mini"><div class="hero-mini-label">Rotas por hora</div><div class="hero-mini-value" style="font-size:22px;" id="heroRotasPorHoraGeral"><?= $rotasPorHoraGeral ?></div></div>
            </div>
        </div>
    </div>
    ...
    <div class="card">
        ...
        <div class="kpis">
            <div class="kpi"><div class="kpi-label">Total</div><div class="kpi-value" id="kpiTotalGeral"><?= $totalGeral ?></div></div>
            <div class="kpi"><div class="kpi-label">Ativos</div><div class="kpi-value" id="kpiTotalAtivos"><?= $totalAtivos ?></div></div>
            <div class="kpi"><div class="kpi-label">Conferindo</div><div class="kpi-value" id="kpiTotalConferindo"><?= $totalConferindo ?></div></div>
            <div class="kpi"><div class="kpi-label">Finalizados</div><div class="kpi-value" id="kpiTotalFinalizados"><?= $totalFinalizados ?></div></div>
        </div>
    </div>
    ...
    <div class="card">
        <div class="bloco-info">
            <h3>Motoristas</h3>
            <p>Filtre rapidamente os registros por status e grupo de veículo.</p>
        </div>

        <div class="table-toolbar">
            <div class="table-toolbar-title">Filtro por status</div>
            <div class="filtros">
        ...
        <div class="table-toolbar" style="margin-top:-4px;">
            <div class="table-toolbar-title">Filtro por veículo</div>
            <div class="filtros">
                ...
                <a href="admin.php?status=<?= urlencode($filtroStatus) ?>&veiculo=fiorino_grupo" class="filtro-btn <?= filtroAtivo($filtroVeiculo, 'fiorino_grupo') ?>">FIORINO / ZOF-FIORINO-AR</a>
                <a href="admin.php?status=<?= urlencode($filtroStatus) ?>&veiculo=passeio_grupo" class="filtro-btn <?= filtroAtivo($filtroVeiculo, 'passeio_grupo') ?>">PASSEIO / ZOF-PASSEIO-AR</a>
            </div>
        </div>
                                ...
                                <td>
                                    <div class="acoes-tabela">
                                        ...
                                        <form method="post" class="form-delete-route" style="margin:0;">
                                            ...
                                            <button type="submit" class="btn-mini delete">Excluir</button>
                                        </form>
                                    </div>
                                </td>
                            </tr>
...
const CURRENT_STATUS_FILTER = <?= json_encode($filtroStatus) ?>;
const CURRENT_VEHICLE_FILTER = <?= json_encode($filtroVeiculo) ?>;

let adminReloadTimer = null;
let dashboardPollingHandle = null;

function recarregarAdminRealtime() {
    if (adminReloadTimer) return;
    adminReloadTimer = setTimeout(async () => {
        adminReloadTimer = null;
        await atualizarDashboardSemReload();
    }, 400);
}
...
adminSupabase
    ...
    .on("postgres_changes", { event: "*", schema: "public", table: "mesas_controle" }, () => recarregarAdminRealtime())
    .subscribe((status) => {
...
async function atualizarDashboardSemReload() {
    try {
        const url = `admin.php?ajax=dashboard_data&status=${encodeURIComponent(CURRENT_STATUS_FILTER)}&veiculo=${encodeURIComponent(CURRENT_VEHICLE_FILTER)}`;
        const resposta = await fetch(url, { cache: 'no-store' });
        const dados = await resposta.json();

        if (!dados.ok) return;

        document.getElementById('heroTotalAtivos').textContent = dados.resumo.totalAtivos;
        document.getElementById('heroTotalConferindo').textContent = dados.resumo.totalConferindo;
        document.getElementById('heroTotalFinalizados').textContent = dados.resumo.totalFinalizados;
        document.getElementById('heroTotalRotasMesas').textContent = dados.resumo.totalRotasMesas;
        document.getElementById('heroTempoMedioGeral').textContent = dados.resumo.tempoMedioGeral;
        document.getElementById('heroRotasPorHoraGeral').textContent = dados.resumo.rotasPorHoraGeral;

        document.getElementById('kpiTotalGeral').textContent = dados.resumo.totalGeral;
        document.getElementById('kpiTotalAtivos').textContent = dados.resumo.totalAtivos;
        document.getElementById('kpiTotalConferindo').textContent = dados.resumo.totalConferindo;
        document.getElementById('kpiTotalFinalizados').textContent = dados.resumo.totalFinalizados;

        const tbody = document.getElementById('driversTableBody');
        if (!tbody) return;

        if (!Array.isArray(dados.drivers) || dados.drivers.length === 0) {
            tbody.innerHTML = '<tr><td colspan="7" class="vazio">Nenhum motorista encontrado para esse filtro.</td></tr>';
            return;
        }

        tbody.innerHTML = dados.drivers.map((d) => `
            <tr>
                <td><span class="tag">${escapeHtml(d.driver_id)}</span></td>
                <td><strong>${escapeHtml(d.driver_name)}</strong></td>
                <td>${escapeHtml(d.cluster_text)}</td>
                <td><strong>${escapeHtml(d.packages_total)}</strong></td>
                <td>${escapeHtml(d.vehicle_type)}</td>
                <td><span class="status ${escapeHtml(d.status_class)}">${escapeHtml(d.status)}</span></td>
                <td>
                    <div class="acoes-tabela">
                        <a class="btn-mini edit" href="editar_motorista.php?id=${Number(d.id)}">Editar</a>
                        <form method="post" class="form-delete-route" style="margin:0;">
                            <input type="hidden" name="action" value="delete_route">
                            <input type="hidden" name="delete_driver_id" value="${Number(d.id)}">
                            <button type="submit" class="btn-mini delete">Excluir</button>
                        </form>
                    </div>
                </td>
        ...
        console.error('Falha ao atualizar dashboard sem reload:', e);
    }
}

function iniciarPollingDashboard() {
    if (dashboardPollingHandle) clearInterval(dashboardPollingHandle);
    dashboardPollingHandle = setInterval(atualizarDashboardSemReload, 30000);
}
